  ...
  = if bound == 0 then return VarLeaf
      ...
      PropEq s t -> 
        do { refl_out  <- fresh_var 
           ...
           ; let e = exp_binop XOr (EVar refl_out)
                                (exp_binop XOr (EVar sym_out)
                                            (EVar trans_out))
           ; cs_of_exp out e
...
compile_refl choose_refl out s t
  = do { let c = exp_binop Eq (exp_of_tm s) (exp_of_tm t)
       ; let e = exp_binop And (EVar choose_refl) c 
       ; cs_of_exp out e
       }
...
compile_sym bound choose_sym out s t
  = do { eq_ts      <- fresh_var -- +1
       ...
       ; let e = exp_binop And (EVar choose_sym) (EVar eq_ts)
       ; cs_of_exp out e
       ; return recur_vars
       }
...
compile_trans bound choose_trans choose_u out s t
  = do { eq_su <- fresh_var 
       ...
       ; let e = exp_binop And (EVar choose_trans)
                            (exp_binop And (EVar eq_su) (EVar eq_ut)) 
       ; cs_of_exp out e
