...
data CEnv a =
  CEnv { cur_cs   :: Set.Set (Constraint a)
       , next_var :: Var
       , var_assgn :: Assgn a -> Assgn a
       }

add_direct_bind :: a -> Var -> State (CEnv a) ()
add_direct_bind c x
  = do { cenv <- get
       ; let f m = let m' = var_assgn cenv m in Map.insert x c m'
       ; put (cenv { var_assgn = f })
       }

add_bind :: Show a => (a -> a -> a) -> a -> [Var] -> Var -> State (CEnv a) ()
add_bind interp_f c_unit xs out
  = modify (\cenv -> cenv { var_assgn = go cenv c_unit (reverse xs) })
  where go cenv c0 [] m
          = Map.insert out c0 (var_assgn cenv m)
        go cenv c0 (x : xs') m 
          = case Map.lookup x (var_assgn cenv m) of
              Nothing -> error $ "expected binding for " ++ show x
                                 ++ " in map " ++ show (var_assgn cenv m)
                                 ++ " for remaining vars = " ++ show xs'
                                 ++ " for constant " ++ show c0
                                 ++ " for cenv " ++ show (cur_cs cenv)
              Just c -> go cenv (c `interp_f` c0) xs' (var_assgn cenv m)
...
get_witness :: State (CEnv a) (Assgn a -> Assgn a)
get_witness
  = do { cenv <- get
       ; return $ var_assgn cenv
       }
...
-- | Add constraint 'x = c'
ensure_const :: Field a => (a,Var) -> State (CEnv a) ()
ensure_const (c,x)
  = do { add_direct_bind c x
       ; add_constraint
         $ CAdd c
...
encode_or (x,y,z)
  = do { x_mult_y <- fresh_var
       ...
       ; add_bind (\c d -> if c == one || d == one then one
                           else zero) zero [x,y] z
       ; add_constraint
         $ CAdd zero
...
encode_xor (x,y,z)
  = do { x_mult_y <- fresh_var
       ...
       ; add_bind (\c d ->
           let res :: Field a => a
               res = if c == one && d == zero then one
                     else if c == zero && d == one then one
                          else zero
           in res) zero [x,y] z
       ; add_constraint
         $ CAdd zero
...
encode_boolean_eq (x,y,z)
  = do { x_mult_y <- fresh_var
       ...
       ; neg_x <- fresh_var
       ; add_bind (\c _ -> one `add` neg c) zero [x] neg_x
       ; add_constraint (CAdd one $ Map.fromList [(x,neg one),(neg_x,neg one)])

       ; neg_y <- fresh_var
       ; add_bind (\c _ -> one `add` neg c) zero [y] neg_y
...
cs_of_exp out e = case e of
  EVar x ->
    do { ensure_equal (x,out)
       }

  EVal c ->
    do { ensure_const (c,out)
       }
...
r1cs_of_constraints :: Field a
                    ...
                    -> ConstraintSystem a 
                    -> R1CS a
r1cs_of_constraints gen_witness cs
  = let pinned_vars = cs_out_vars cs ++ cs_in_vars cs
        ...
        f m = Map.mapKeys renumber_f $ gen_witness m 
    in r1cs_of_cs cs' f
...
compile_exp nv in_vars te
  = let out = nv 
        ...
        cenv_init = CEnv Set.empty (out+1) id
